...
import { ChevronLeft, ChevronRight, Plus, DoorOpen } from 'lucide-react';
    ...
    return roomSchedules
      .filter(schedule => {
        ...
        return scheduleDate.toDateString() === date.toDateString() &&
               schedule.equipment?.some((eq: any) => eq.id === roomId && eq.type === 'room');
      })
      .map(schedule => ({
        ...schedule,
                  ...
                  return (
                    <td key={i} className="px-3 py-4 whitespace-nowrap text-sm text-gray-500 relative group border border-gray-100">
                      <button
                        ...
                        className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-emerald-100 rounded-full p-1"
                      >
                      ...
                      <div className={view === 'day' ? 'min-h-[120px]' : 'min-h-[80px]'}>
                        {schedules.map(schedule => (
                          ...
                          >
                            ...
                            {schedule.details && (
                              <div className="text-xs text-emerald-500 truncate">{schedule.details}</div>
                            )}
                          </div>
                        ))}
                      </div>
                    ...
                    {daySchedules.slice(0, 3).map(schedule => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-xs text-emerald-600 truncate mt-1">
                            {getParticipantNames(schedule.participants || [])}
                          </div>
                        </div>
                      );
